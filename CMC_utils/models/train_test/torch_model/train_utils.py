...
from CMC_utils.models import remove_DataParallel_module
...
class ModelCheckpointer:
    ...
    def __init__(self, model, optimizer, best_epoch: int = -1, best_loss: float = None, best_performance: list = None ):
        if isinstance(model, torch.nn.DataParallel):
            self.best_model_weights = model.module.state_dict()
        else:
            self.best_model_weights = model.state_dict()
        self.best_optimizer_weights = optimizer.state_dict()
...
class CustomScheduler:
    ...
    """
    def __init__(self, learning_rates: list, patience: int, verbose: int, min_epochs: int, model_parts: dict = None, epochs_no_improve: int = 0, learning_rate_idx: int = 0, idx_reverse_order: bool = False, invert_order_on_lr_end: bool = False):

        self.learning_rates = learning_rates
        self.patience = patience
        self.verbose = verbose
        self.min_epochs = min_epochs
        # self.reverse_order = reverse_order  # reverse_order: bool = True,
        self.model_parts = model_parts
        self.invert_order_on_lr_end = invert_order_on_lr_end

        self.state = dict(epochs_no_improve=epochs_no_improve, learning_rate_idx=learning_rate_idx, idx_reverse_order=idx_reverse_order)
    ...
    @property
    def lr(self):
        lr_idx = max(min(self.state["learning_rate_idx"], len(self.learning_rates) - 1), 0)
        if self.model_parts is not None:
            model_params = dict()
            for k, v in self.model_parts.items():
                if isinstance(v, (list, ListConfig)):
                    # if isinstance(v[0], (list, ListConfig)):
                    #    model_params[k] = [v[i][min(lr_idx, len(v[i])-1)] for i in range(len(v))]
                    # else:
                    model_params[k] = v[min(lr_idx, len(v)-1)]
                else:
                    model_params[k] = v
            # model_params = {k: v if not isinstance(v, (list, ListConfig)) else v[min(lr_idx, len(v)-1)] for k, v in self.model_parts.items()}
            return model_params
        return {"all": self.learning_rates[lr_idx]}
    ...
    def update_state(self, epoch: int):
        ...
        if epoch > self.min_epochs:
            ...
            if self.next_step:
                if self.invert_order_on_lr_end:
                    if not self.state["idx_reverse_order"]:
                        self.state["learning_rate_idx"] += 1
                        self.state["idx_reverse_order"] = False if self.state["learning_rate_idx"] < len(self.learning_rates) - 1 else True
                    else:
                        self.state["learning_rate_idx"] -= 1
                        self.state["idx_reverse_order"] = True if self.state["learning_rate_idx"] > 0 else False
                    print_options[self.verbose](f"Learning rate changed to {self.lr}")
                elif self.state["learning_rate_idx"] < len(self.learning_rates) - 1:
                    self.state["learning_rate_idx"] += 1
                    print_options[self.verbose](f"Learning rate changed to {self.lr}")
...
class TrainManager:
    ...
    def __init__(self, model, filename: str, path: str, min_epochs: int, max_epochs: int, metrics: dict, early_stop: dict, model_checkpoint: dict, scheduler: dict, optimizer: dict, model_params: dict, performance_tolerance: float = 1e-3, device: str = "cpu", verbose: int = 0, **_ ):
        ...
        # model_parts = [{"params": eval(f"model{model_txt}.parameters()"), "lr": lr} if part == "all" else {"params": eval(f"model.{part}.parameters()"), "lr": lr} for part, lr in self.scheduler.lr.items()]
        model_parts = [{"params": model.parameters(), "lr": lr} if part == "all" else {"params": eval(f"model{model_txt}.{part}").parameters(), "lr": lr} for part, lr in self.scheduler.lr.items()]
        self.optimizer = instantiate(self.optimizer_params["init"], model_parts)  # , lr=self.scheduler.lr)
        # self.optimizer = instantiate(self.optimizer_params["init"], model_parts)
    ...
    def load_checkpoint(self, model: torch.nn.Module) -> torch.nn.Module:
            ...
            if os.path.exists(checkpoint_path + "_checkpoint.pkl"):
                ...
                optimizer_state_dict = torch.load(checkpoint_path + "_optimizer.pth", map_location=self.device, weights_only=False)
                ...
                model_state_dict = torch.load(checkpoint_path + f".{self.model_params['file_extension']}", map_location=self.device)
    ...
    def callbacks(self, model, epoch: int, epoch_loss: float, epoch_performance: float = None) -> None:
        ...
        if epoch <= self.min_epochs:
            ...
            performance_improved = (epoch_performance - self.model_checkpoint.state["best_performance"]) >= self.tol and (self.model_checkpoint.state["best_loss"] - epoch_loss) > self.tol
        else:

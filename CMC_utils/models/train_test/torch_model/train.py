...
from CMC_utils.miscellaneous import do_nothing, do_really_nothing, seed_worker
...
def train_torch_model(model: torch.nn.Module, train_set: SupervisedTabularDatasetTorch, model_params: dict, model_path: str, val_set: SupervisedTabularDatasetTorch, train_params: Union[dict, DictConfig], test_fold: int = 0, val_fold: int = 0, use_weights: bool = True, **kwargs) -> None:
    ...
    """
    if use_weights:
        if train_set.label_type in ("single_risk_survival", "competing_risks_survival"):
            labels = train_set.labels[:, 0]
        else:
        ...
        train_dataloader = DataLoader(train_set, batch_size=train_params.dl_params.batch_size, sampler=sampler, drop_last=True, worker_init_fn=seed_worker, num_workers=4, pin_memory=True)
    else:
        train_dataloader = DataLoader(train_set, batch_size=train_params.dl_params.batch_size, shuffle=True, drop_last=True, worker_init_fn=seed_worker, num_workers=4, pin_memory=True)

    val_dataloader = DataLoader(val_set, batch_size=train_params.dl_params.batch_size, shuffle=False, drop_last=False, num_workers=4, pin_memory=True)
    dataloaders = dict(train=train_dataloader, val=val_dataloader)

    if train_params.dl_params.initialize_weights:
    ...
    regularizers = [instantiate(regularizer_params["init_params"]).to(tr_manager.device) for regularizer_params in train_params.get("regularizer", {}).values()]
    ...
    for epoch in tr_manager.epochs():
        ...
        for phase, dataloader in dataloaders.items():
            ...
            epoch_performance = metrics_computation_df(epoch_results, metrics=metrics, task=dataloader.dataset.task, classes=dataloader.dataset.classes, use_weights=True, verbose=0)  # TODO solve regression metrics calculation
...
def model_predict(model, dataloader, criterions, regularizers, optimizer, tr_manager, train_params, phase: str = "val", **kwargs) -> Tuple[torch.nn.Module, float, pd.DataFrame]:
    ...
    with torch.set_grad_enabled(phase == 'train'):
        for *input_list, labels, idxs in pbar:
            input_list = [inputs.float().to(tr_manager.device, non_blocking=True) for inputs in input_list]
            labels = labels.float().to(tr_manager.device, non_blocking=True)
            ...
            elif dataloader.dataset.label_type == "multilabel":
                outputs = F.sigmoid(outputs)

            if dataloader.dataset.label_type in ("single_risk_survival", "competing_risks_survival"):
                preds = output_to_pred_options[dataloader.dataset.label_type](outputs, return_first=True, num_events=model.num_events, max_time=model.max_time, **kwargs)
            else:
            ...
            elif labels.shape[-1] != preds.shape[-1]:
            ...
            if dataloader.dataset.label_type == "continuous":
                preds = list(np.round(preds, dataloader.dataset.decimals))
            else:
            ...
            if dataloader.dataset.label_type != "categorical":
                outputs = np.round(outputs, getattr(dataloader.dataset, "decimals", 4))
            ...
            batch_results = pd.DataFrame(dict( ID=idxs, label=labels, prediction=preds, probability=outputs.tolist()))

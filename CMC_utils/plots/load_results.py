...
def load_missing_results(results_path: str, balanced: bool = True, result_type: str = "set", separate_experiments: bool = True, verbose: bool = False) -> pd.DataFrame:
    ...
    result_type_options = dict(set="set_average_performance.csv", classes="classes_average_performance.csv", folds="all_test_performance.csv", averages="averages_classes_average_performance.csv")
    result_type_args_options = dict(set=dict(header=[0, 1], index_col=0), classes=dict(header=[0, 1], index_col=[0, 1]), folds=dict(header=[0]), averages=dict(header=[0, 1], index_col=[0, 1]))
    index_cols_names = dict(set=["set"], classes=["set", "class"], folds=["set", "class", "test_fold", "val_fold"], averages=["set", "class"])
    all_results = pd.DataFrame()
        ...
        for train_percentage in train_percentages:
            ...
            for test_percentage in test_percentages:
                ...
                for approach in late_fusion_approaches:
                    ...
                    results = results.assign(experiment=exp_name, train_percentage=train_percentage, test_percentage=test_percentage)
                    results = results.assign(db=cfg["db_name"], model=cfg["model_name"], imputer=cfg["preprocessing"].get("imputer", {}).get("method", ""))
                    results = results.assign(missing_strategy=cfg.get("multimodal_missing_generation", ""))
                    fusion_strategies = {"multimodal_early_fusion": "early", "multimodal_late_fusion": "late", "multimodal_joint_fusion": "joint", "missing": "-", "simple": "-"}

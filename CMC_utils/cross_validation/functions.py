...
def provided(data: np.array, labels: np.array, sets: List[str], path: str, test_fold: str = None, **kwargs) -> Tuple[int, np.array, np.array]:
    try:
        for test_fold, val_fold, train_info, val_info, test_info, _ in get_cross_validation(path, "train", "val", "test", specific_fold=test_fold, print_fold=False):
            if "test" in sets:
                train_info = pd.concat([train_info, val_info], axis=0)
                yield test_fold, train_info.idx.values, test_info.idx.values
            else:
                # train_info, val_info = train_info.reset_index().set_index("ID"), val_info.reset_index().set_index("ID")
                train_val_index = pd.Series(data, name="ID").reset_index().set_index("ID")
                yield val_fold, train_val_index.loc[train_info.ID, "index"].values, train_val_index.loc[val_info.ID, "index"].values
    except FileNotFoundError as e:
        for test_fold, val_fold, train_info, test_info, _ in get_cross_validation(path, "train", "test", specific_fold=test_fold, print_fold=False):
            yield test_fold, train_info.idx.values, test_info.idx.values
...
def set_cross_validation(info_for_cv: pd.DataFrame, path: str, test_params: dict, val_params: dict, force: bool = False, **kwargs) -> None:
    ...
    options = {"holdout": holdout, "multilabelholdout": multilabelholdout, "kfold": kfold, "stratifiedkfold": stratifiedkfold, "multilabelstratifiedkfold": multilabelstratifiedkfold, "loo": leaveoneout,
               "bootstrap": bootstrap, "multilabelbootstrap": multilabelbootstrap, "predefined": predefined, "provided": provided}
    ...
    test_cv = options[test_params["method"]](info_for_cv.ID.values, info_for_cv.target.values, sets=["train", "test"], **test_params)

    for fold, train_val_index, test_index in test_cv:
        # Create DataFrame containing information about the test set
        if not test_params["method"].startswith("multilabel"):
            test_info = pd.DataFrame({"idx": test_index, "ID": info_for_cv.ID[test_index].values, "target": info_for_cv.target.iloc[test_index].values})
            ...
            test_labels = info_for_cv.target.iloc[test_index].reset_index(drop=True)
            test_info = pd.concat( [pd.DataFrame({"idx": test_index, "ID": info_for_cv.ID[test_index].values}), test_labels], axis=1 )
            # Split training and validation data using the validation cross-validation method
            train_val_IDs, train_val_info = info_for_cv.ID[train_val_index], info_for_cv.target.iloc[train_val_index]  # .reset_index().rename({"index": "original_index"}, axis=1)
            train_val_info.columns = pd.MultiIndex.from_product([["target"], train_val_info.columns])
            train_val_info = train_val_info.reset_index().rename({"index": "original_index"}, axis=1)

        val_cv = options[val_params["method"]](train_val_IDs.values, train_val_info.target.values, sets=["train", "val"], test_fold=f"{fold}_0", **val_params)
        ...
        for val_fold, train_index, val_index in val_cv:
            ...
            if not test_params["method"].startswith("multilabel"):
                ...
                train_info = pd.DataFrame({"idx": train_original_index.values, "ID": info_for_cv.ID.iloc[train_original_index].values, "target": info_for_cv.target.iloc[train_original_index].values})

                # Create DataFrame containing information about the validation set
                val_info = pd.DataFrame({"idx": val_original_index.values, "ID": info_for_cv.ID.iloc[val_original_index].values, "target": info_for_cv.target.iloc[val_original_index].values})
            else:
                # Create DataFrame containing information about the train set
                train_labels = info_for_cv.target.iloc[train_original_index].reset_index(drop=True)
                train_info = pd.concat([pd.DataFrame({"idx": train_original_index.values, "ID": info_for_cv.ID.iloc[train_original_index].values}), train_labels], axis=1)

                # Create DataFrame containing information about the validation set
                val_labels = info_for_cv.target.iloc[val_original_index].reset_index(drop=True)
                val_info = pd.concat([pd.DataFrame({"idx": val_original_index.values, "ID": info_for_cv.ID.iloc[val_original_index].values}), val_labels], axis=1)

            fold_path = os.path.join( path, f"{fold}_{val_fold}")
            os.makedirs(fold_path, exist_ok=True)
            train_info, val_info, test_info = train_info.replace(-100, np.nan), val_info.replace(-100, np.nan), test_info.replace(-100, np.nan)
...
def get_cross_validation(path: str, *sets: str, print_fold: bool = True, specific_fold: str = None) -> Tuple[int, int, pd.DataFrame, pd.DataFrame, pd.DataFrame, bool]:
    """
    ...
    if specific_fold is not None:
        folds_paths = {i: f for i, f in enumerate(folds_paths) if f.endswith(specific_fold)}
        folds = folds.iloc[list(folds_paths.keys())]
        folds_paths = list(folds_paths.values())

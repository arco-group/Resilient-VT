...
def impute_missing(data: pd.DataFrame, features_info: Union[dict, pd.Series], method: str, return_mask: bool = False,
                   return_imputer: bool = False, imputer: object = None, concat_mask: bool = False, skipna_samples: bool = True, **kwargs) -> Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame], Tuple[pd.DataFrame, object], Tuple[pd.DataFrame, pd.DataFrame, object]]:
    """
    ...
    Parameters:
        ...
        method: str.
            The imputation method to use. Can be one of 'simple', 'iterative', 'knn', or 'no'.
                - 'simple': use scikit-learn's SimpleImputer.
                - 'iterative': use scikit-learn's IterativeImputer.
                - 'knn': use scikit-learn's KNNImputer.
                - 'no': don't impute missing values, just return a mask indicating which values are missing.
        ...
        skipna_samples: bool, default=False.
            Whether to skip the samples with missing values or not.
        **kwargs:
    ...
    try:
        options = dict(simple=SimpleImputer, iterative=IterativeImputer, knn=KNNImputer, noimputation=MissingIndicator, missforest=MissForest)
    except NameError as _:
        options = dict(simple=SimpleImputer, iterative=IterativeImputer, knn=KNNImputer, noimputation=MissingIndicator)
    ...
    if method != "noimputation":
        data_filled = imputer.transform(data_filled)  # , index=data_filled.index, columns=dataset_columns)
        ...
        # round the filled values in discrete columns
        # discrete_columns = features_info[features_info == "int"].index.tolist()
        # data_filled[discrete_columns] = data_filled[discrete_columns].round(0)

        # argmax the filled values in categorical columns
        ...
        for cat_column in categorical_columns:
            ...
            if len(relative_columns) == 1:
                ...
                data_filled[relative_columns] = data_filled[relative_columns].clip(0, len(features_info["categorical_unique_values"].loc[relative_columns]))  # np.ma.masked_invalid(data_filled[relative_columns]).max(axis=0))
    ...
    if skipna_samples:
        data_filled.loc[(mask.sum(axis=1) == mask.shape[1]).values] = np.nan
